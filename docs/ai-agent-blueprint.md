...
> **Leitura ampliada (23/06/2026, v0.19.1).** +3 read tools (total 17): `get_card_purchase_details` (plano de parcelas de uma compra), `get_card_statement_details` (itens + pagamentos de uma fatura) e filtro `onlyUncategorized` em `search_account_movements` (atende o caso "lançamentos sem categoria" da Seção 4.1; saída agora inclui `categoryId`/`hasCategory`). 3 testes novos.
