...
import com.osiris.mobile.presentation.accounts.MovementFormEvent
...
fun MovementFormScreen(
    accountId: String,
    onDone: () -> Unit,
    viewModel: MovementFormViewModel = koinViewModel { parametersOf(accountId) },
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                MovementFormEvent.NavigateBack -> onDone()
                is MovementFormEvent.ShowMessage -> snackbarHostState.showSnackbar(event.message)
            }
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.movement_new)) },
                navigationIcon = {
        ...
        Column(
            ...
            Text(stringResource(R.string.movement_type_label), style = MaterialTheme.typography.labelLarge)
            Spacer(Modifier.height(8.dp))
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                FilterChip(
                    selected = state.type == MovementType.Income,
                    onClick = { viewModel.onTypeChange(MovementType.Income) },
                    label = { Text(stringResource(R.string.movement_income)) },
                )
                FilterChip(
                    selected = state.type == MovementType.Expense,
                    onClick = { viewModel.onTypeChange(MovementType.Expense) },
                    label = { Text(stringResource(R.string.movement_expense)) },
                )
            }
            ...
            OsirisTextField(
                ...
                label = stringResource(R.string.movement_amount_label),
                error = state.amountError,
                keyboardType = KeyboardType.Decimal,
            )
            Spacer(Modifier.height(20.dp))
            OsirisDateField(
                label = stringResource(R.string.movement_date_label),
                value = state.occurredOn,
                onValueChange = viewModel::onOccurredOnChange,
            )
            Spacer(Modifier.height(20.dp))
            OsirisTextField(
                value = state.description,
                onValueChange = viewModel::onDescriptionChange,
                label = stringResource(R.string.movement_description_label),
            ...
            OsirisTextField(
                ...
                label = stringResource(R.string.movement_notes_label),
                error = null,
            )
